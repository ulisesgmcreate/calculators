...
class TettoiaCalculator(tk.Tk):
    def __init__(self):
        ...
        # Fattori di zoom - Default a 2.0x
        self.zoom_factor_top = 2.0
        self.zoom_factor_iso = 2.0
    ...
    def setup_ui(self):
        ...
        self.zoom_factor_top_var.set("2.0x")  # Default a 2.0x
        ...
        # Frame per contenere il canvas e le scrollbar per la vista dall'alto
        canvas_frame_top = ttk.Frame(self.tab_top)
        canvas_frame_top.pack(fill=tk.BOTH, expand=True)
        
        # Canvas per vista dall'alto
        self.canvas = tk.Canvas(canvas_frame_top, bg=self.color_input_bg, height=300)
        self.canvas.grid(row=0, column=0, sticky='nsew')
        
        # Scrollbar orizzontale per vista dall'alto
        h_scrollbar_top = ttk.Scrollbar(canvas_frame_top, orient=tk.HORIZONTAL, command=self.canvas.xview)
        h_scrollbar_top.grid(row=1, column=0, sticky='ew')
        
        # Scrollbar verticale per vista dall'alto
        v_scrollbar_top = ttk.Scrollbar(canvas_frame_top, orient=tk.VERTICAL, command=self.canvas.yview)
        v_scrollbar_top.grid(row=0, column=1, sticky='ns')
        
        # Configura il canvas per usare le scrollbar
        self.canvas.configure(xscrollcommand=h_scrollbar_top.set, yscrollcommand=v_scrollbar_top.set)
        
        # Configura il grid per espandersi
        canvas_frame_top.grid_rowconfigure(0, weight=1)
        canvas_frame_top.grid_columnconfigure(0, weight=1)
        ...
        self.zoom_factor_iso_var.set("2.0x")  # Default a 2.0x
        ...
        # Frame per contenere il canvas e le scrollbar per la vista 3D
        canvas_frame_iso = ttk.Frame(self.tab_iso)
        canvas_frame_iso.pack(fill=tk.BOTH, expand=True)
        
        # Canvas per vista 3D
        self.canvas_iso = tk.Canvas(canvas_frame_iso, bg=self.color_input_bg, height=350)
        self.canvas_iso.grid(row=0, column=0, sticky='nsew')
        
        # Scrollbar orizzontale per vista 3D
        h_scrollbar_iso = ttk.Scrollbar(canvas_frame_iso, orient=tk.HORIZONTAL, command=self.canvas_iso.xview)
        h_scrollbar_iso.grid(row=1, column=0, sticky='ew')
        
        # Scrollbar verticale per vista 3D
        v_scrollbar_iso = ttk.Scrollbar(canvas_frame_iso, orient=tk.VERTICAL, command=self.canvas_iso.yview)
        v_scrollbar_iso.grid(row=0, column=1, sticky='ns')
        
        # Configura il canvas per usare le scrollbar
        self.canvas_iso.configure(xscrollcommand=h_scrollbar_iso.set, yscrollcommand=v_scrollbar_iso.set)
        
        # Configura il grid per espandersi
        canvas_frame_iso.grid_rowconfigure(0, weight=1)
        canvas_frame_iso.grid_columnconfigure(0, weight=1)
    ...
    def reset_zoom(self, view_type):
        ...
        if view_type == "top":
            self.zoom_factor_top = 2.0  # Default a 2.0x
            self.zoom_factor_top_var.set("2.0x")
            if self.last_calc_data:
                self.disegna_layout(
                    self.last_calc_data['larghezza'],
                    self.last_calc_data['lunghezza'],
            ...
            self.zoom_factor_iso = 2.0  # Default a 2.0x
            self.zoom_factor_iso_var.set("2.0x")
            if self.last_calc_data:
                self.disegna_vista_isometrica(
                    self.last_calc_data['larghezza'],
                    self.last_calc_data['lunghezza'],
    ...
    def disegna_layout(self, larghezza, lunghezza, num_pali_x, num_pali_y, num_posti_auto=0, altezza=220, num_pannelli=0):
        ...
        # Calcola le dimensioni totali del disegno dopo lo zoom
        total_width = larghezza * scala + 120  # Extra spazio per margini
        total_height = lunghezza * scala + 120  # Extra spazio per margini e informazioni
        
        # Configura il canvas per lo scrolling
        self.canvas.configure(scrollregion=(0, 0, total_width, total_height))
        
        # Offset per centrare il disegno
        offset_x = (total_width - (larghezza * scala)) / 2
        offset_y = 60  # Spazio per il titolo
    ...
    def disegna_vista_isometrica(self, larghezza, lunghezza, altezza, num_pali_x, num_pali_y, num_posti_auto, num_pannelli):
        ...
        # Calcola le dimensioni totali del disegno dopo lo zoom
        # In vista isometrica, lo spazio necessario è maggiore
        max_projected_width = (larghezza + lunghezza) * cos_a * scala * 1.2
        max_projected_height = (larghezza + lunghezza) * sin_a * scala + altezza * scala * 1.2
        
        total_width = max(canvas_width, max_projected_width) + 100  # Extra spazio per margini
        total_height = max(canvas_height, max_projected_height) + 120  # Extra spazio per margini e informazioni
        
        # Configura il canvas per lo scrolling
        self.canvas_iso.configure(scrollregion=(0, 0, total_width, total_height))
        
        # Offset per centrare il disegno
        offset_x = total_width * 0.45
        offset_y = 60  # Spazio per il titolo
